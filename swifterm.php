 ...
 * Description: SwiftERM automation tool for WooCommerce.
 * Version: 1.2.0
 * Author: SwiftERM
	...
	* @version	1.2.0
	...
	class swifterm {
		...
		function webtracking_process_order($order_id){
			if (get_option(self::OPTION_API_KEY)) {
				...
				foreach ($items as $item) {
					...
					$_product[] = new SwiftAPI_Product($item['product_id'], $item['qty'], $product->get_price_including_tax());
				}
				$_SESSION[self::SWIFTERM_ORDER_NAME] = base64_encode(serialize(new SwiftAPI_Request_Order($_SERVER['HTTP_HOST'], $this->generate_user_id(), $this->get_swifterm_email($order->billing_email), $order->billing_first_name, $order->billing_last_name, $_product, $order_id, null, null, $order->order_date)));
			}
		}
			...
			if (get_option(self::OPTION_API_KEY)) {
				...
				if (!$this->request_is_frontend_ajax()) {
					...
					if (isset($_SESSION[self::SWIFTERM_ORDER_NAME])) {
						...
						$request = new SwiftAPI_Request_Home($_SERVER['HTTP_HOST'], $this->generate_user_id(), $_SERVER['REQUEST_URI'], $this->get_swifterm_email());
			...
			if (get_option(self::OPTION_API_KEY)) {
				...
				if ( sizeof( WC()->cart->get_cart() ) > 0 ) {
					...
					foreach ( WC()->cart->get_cart() as $cart_item_key => $values ) {
						$_product[] = new SwiftAPI_Product($values['product_id'], $values['quantity'], $values['data']->get_price_including_tax());
					}
					$_SESSION[self::SWIFTERM_CART_NAME] = base64_encode(serialize(new SwiftAPI_Request_Cart($_SERVER['HTTP_HOST'], $this->generate_user_id(), $_product, $this->get_swifterm_email())));
				}
		...
		function webtracking_product() {
			global $post;
			if (get_option(self::OPTION_API_KEY)) {
				$request = new SwiftAPI_Request_Product($_SERVER['HTTP_HOST'], $this->generate_user_id(), $post->ID, $this->get_swifterm_email());
				echo SwiftAPI::Script($request, hex2bin(get_option(self::OPTION_API_KEY)));
			}
		}
