	...
	public function generate_xml() {
		...
		while ( $loop->have_posts() ) : $loop->the_post();
			...
			if ($post_meta['_stock_status'][0] == 'instock') {
				...
				$method = 'g:price';
				$tempXml[] = $xml -> $method(base64_encode($product->get_price_including_tax(1,$post_meta['_regular_price'][0])));
				$method = 'g:sale_price';
				
				$special_price = '';
				if ($product->is_on_sale() && $product->get_regular_price()) {
					$special_price = $product->get_price_including_tax(1,$post_meta['_sale_price'][0]);
				}
